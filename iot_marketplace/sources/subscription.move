...
public struct SubscriptionTransferred has copy, drop {
    subscription_id: ID,
    old_consumer: address,
...
public entry fun subscribe_to_feed(
    ...
    // Transfer subscription to consumer (OWNED object)
...
public entry fun execute_query(
    ...
    assert!(subscription.consumer == ctx.sender(), ENotSubscriber);
    assert!(subscription.is_active, ESubscriptionInactive);
    assert!(ctx.epoch() <= subscription.expiry_epoch, ESubscriptionExpired);
...
/// Transfer a subscription to another consumer address
public entry fun transfer_subscription(
    ...
    let subscription_id = object::id(&subscription);
    let old_consumer = subscription.consumer;
    
    // Update consumer field
    subscription.consumer = new_consumer;
    
    event::emit(SubscriptionTransferred {
        subscription_id,
        old_consumer,
        new_consumer,
    });
