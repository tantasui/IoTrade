...
4. Run `register-feed` to create a feed, then set `DATA_FEED_ID`
...
3. Server uploads data to Walrus → gets blob ID
4. Server calls `update_feed_data` on Sui with new blob ID
5. Data is stored in-memory for quick queries
6. Buyers can discover and subscribe to the feed on-chain
